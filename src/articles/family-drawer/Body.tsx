import { WritingShell } from "@/articles/writing-shell"
...
export function FamilyDrawerBody() {
  return (
    <WritingShell>
      <div className="flex flex-col gap-5 px-5 md:px-0">
        ...
        <p className="text-paragraph mb-2 max-w-2xl">
          It feels like whenever the topic of animations (whether that is web or otherwise) comes
          up, the{" "}
        ...
        <p className="text-paragraph max-w-2xl">
          Naturally, the app is filled with lots of great interactions, but here, I will focus on
          just one of them, a wallet options drawer:
        </p>
        <div className="mx-auto my-4 flex max-w-2xl gap-6">
          ...
          <p className="hidden text-[16px] text-gray-500/60 md:block dark:text-gray-400/60">
            ...
            closes the drawer and opens another confirmation dialog, so I did not include that in
            the video.
          </p>
        </div>
        <p className="text-paragraph max-w-2xl">
...
import useMeasure from "react-use-measure"`}</CodeBlock>
        <p className="text-paragraph max-w-2xl">
          ...
          primitives, like <span className="code-inline">motion.div</span> which allows us to
          animate pretty much every property of that element using{" "}
...
const [elementRef, bounds] = useMeasure();`}</CodeBlock>
        <p className="text-paragraph max-w-2xl">
          ...
          <span className="code-inline">bounds</span> (an object containing the element's
          dimensions, including height, width, and position).
        </p>
        <p className="text-paragraph max-w-2xl">
        ...
        <p className="text-paragraph max-w-2xl">
          Moving onto the most important part of the component, an animated container which holds
          the views and decides how to animate between them:
        </p>
...
>
    ...
    <AnimatePresence initial={false} mode="popLayout" custom={view}>
      ...
      >
        ...
        <p className="text-paragraph max-w-2xl">
          ...
          <span className="code-inline">mode="popLayout"</span>, so that when a view is exited, it
          is "popped" out of the DOM, so it does not cause a layout shift by interacting with the
          new view which is animating in. The <span className="code-inline">custom={`{view}`}</span>{" "}
          prop passes the current{" "}
          <InlineDefinition
        ...
        <p className="text-paragraph max-w-2xl">
          ...
          <span className="code-inline">animate</span> state brings it to full opacity, normal
          scale, and removes the blur. The <span className="code-inline">exit</span> animation
          reverses this process, with separate timing for opacity (190ms) versus other properties
          (270ms) to have less overlap between the two views.
        </p>
        <p className="text-paragraph max-w-2xl">
        ...
        <p className="text-paragraph max-w-2xl">
          ...
          (which is an alternative to the "SF Pro Rounded" font that I presume is used in the
          original app) as well as the icon library{" "}
          <span className="code-inline">lucide-react</span>.
        </p>
        <p className="text-paragraph max-w-2xl">
  ...
  <RecoveryView key="recovery" ... />,
        ...
        <p className="text-paragraph max-w-2xl">
          ...
          resize itself, no other changes need to be made to the parent, since we could place
          anything in the views and it would fit.{" "}
